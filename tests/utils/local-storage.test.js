import LocalStorage, {LOCAL_STORAGE_KEY_PREFIX} from '@utils/local-storage';

describe('local-storage', function () {
    describe('set', function () {
        test('should return false if something was wrong', function () {
            const result1 = LocalStorage.set();
            const result2 = LocalStorage.set(100500);
            const result3 = LocalStorage.set('key');
            const result4 = LocalStorage.set(undefined, 'value');
            const result5 = LocalStorage.set('key', window);
    ...
    describe('get', function () {
        test('should return undefined if wrong argument passed', function () {
            const result1 = LocalStorage.get();
            const result2 = LocalStorage.get(100500);
            const result3 = LocalStorage.get([1, 2, 'a', 3]);
            const result4 = LocalStorage.get({ foo: 'bar' });
        ...
        test('should return undefined if value tried to parse isnt string, somehow', function () {
            localStorage.setItem(`${LOCAL_STORAGE_KEY_PREFIX}foo`, { a: 'some value' });

            const result = LocalStorage.get('foo');
    ...
    describe('remove', function () {
        test('should return false if something was wrong', function () {
            const originalMethod = window.localStorage.__proto__.removeItem;

            const result1 = LocalStorage.remove();
            const result2 = LocalStorage.remove(100500);

            window.localStorage.__proto__.removeItem = null;
            const result3 = LocalStorage.remove('foo');

            expect(result1).toEqual(false);
            expect(result2).toEqual(false);
            expect(result3).toEqual(false);

            window.localStorage.__proto__.removeItem = originalMethod;
        });
    ...
    describe('export', function () {
        test('should return undefined for wrong argument', function () {
            const result1 = LocalStorage.export();
            const result2 = LocalStorage.export('123');
            const result3 = LocalStorage.export(100500);
            const result4 = LocalStorage.export({ foo: 'bar' });
    ...
    describe('import', function () {
        test('should return undefined for wrong argument', function () {
            const result1 = LocalStorage.import();
            const result2 = LocalStorage.import(null);
            const result3 = LocalStorage.import(100500);
            const result4 = LocalStorage.import({ foo: 'bar' });
