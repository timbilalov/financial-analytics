...
export function prepareSingleDataset(title, data, amount, isUsd, datasets, datesFullArray, datasetsColors, calcMethod, usdData) {
    ...
    if (title.toLowerCase() === 'total' || title.toLowerCase() === 'bank depo' || title.toLowerCase() === 'own money') {
        ...
        calculatedData = calcData(title, data, amount, isUsd, calcMethod, usdData);
        shouldStoreAbsTotal = true;
        calculatedDataAbsTotal = calcData(title, data, amount, isUsd, CALC_METHODS.ABSOLUTE_TOTAL, usdData);
    }
